...
# Pandas 표기 설정
pd.options.display.float_format = '{:.4f}'.format  # 소수점 4자리 까지 표기
pd.set_option('display.max_columns', None)  # Dataframe 출력시 컬럼 생략 없이 전체 표기

df_nasdaq = fdr.StockListing('NASDAQ')
nasdaq = df_nasdaq.Symbol.tolist()  # 나스닥 상장 종목명 로드
nasdaq_list = nasdaq[:1000]  # 1000번째 종목 까지 가져옴
...
closing_price.to_csv('closing_Price(Nasdaq).csv', float_format='%.4f')  # 소수점 4번째 자리 까지만 출력
print(nasdaq_list)
